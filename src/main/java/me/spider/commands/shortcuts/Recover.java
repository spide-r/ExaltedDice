...
public class Recover extends SlashCommand {
    ...
    protected void execute(SlashCommandEvent event) {
        ...
        if(recoverResponse == -1){
            ...
            event.reply("Your peripheral essence is now: `" + ch.getPeripheralMotes() + "`.").queue();
        } else if(recoverResponse == 1){
            event.reply("Your peripheral essence is now `" + ch.getPeripheralMotes() + "` and your personal essence is now `" + ch.getPersonalMotes() + "`.").queue();
        }
    }
}
