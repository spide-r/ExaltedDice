...
public class ModifyAttributes extends SlashCommand {
    public ModifyAttributes(){
        this.name = "modify";
        this.help = "This modifies all of your sheet attributes.";
        for (String s : Constants.ATTRIBUTE_LIST) {
            this.options.add(new OptionData(OptionType.INTEGER, s.toLowerCase(), "Modifies your " + s + " by the specified value"));
        }
    ...
    protected void execute(SlashCommandEvent event) {
        ...
        for (OptionMapping option : event.getOptionsByType(OptionType.INTEGER)) {
            ...
            int oldAmt = ch.getInt(attribute);
            ch.setInt(attribute, oldAmt + amount);
            builder.append("Your ").append(attribute).append(" is now: ").append(oldAmt + amount).append(".\n");
        }
        try {
            c.saveCharacter(ch);
