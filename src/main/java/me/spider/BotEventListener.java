...
public class BotEventListener extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        if(!event.isFromGuild()){
            if(event.getAuthor().getId().equals("102845358677176320")){
                if (event.getMessage().getContentRaw().equals("update")) {
    ...
    private void updateCommands(List<Guild> guilds){
        guilds.forEach(g -> g.updateCommands().addCommands(commandData).queue());
    }

    private static final CommandData[] commandData = new CommandData[]{Commands.slash("roll", "Rolls The Dice - 10's count as 2 successes")
